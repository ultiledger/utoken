    ...
    >
      ...
      <pl-content-block :offsetTop="46" class="b-white">
        <div style="padding-top: 10px;padding-left: 20px;text-align: center;">
          <span
            class="bs-btn buy-btn"
            :class="bsFlag"
            @click.stop="toBs('buy')"
            v-text="$t('trade.buying')"
          ></span>
          <span
            class="bs-btn sell-btn"
            :class="bsFlag"
            @click.stop="toBs('sell')"
            v-text="$t('trade.selling')"
          ></span>
        </div>
          ...
          <div class="item">
            <van-row>
              <van-col span="11" class="x-small-font">
                <span v-if="bsFlag === 'buy'" v-text="$t('trade.buyPrice')"></span>
                <span v-else-if="bsFlag === 'sell'" v-text="$t('trade.sellPrice')"></span>
                ({{tradepair.counterCode}})
              </van-col>
              <van-col
                span="11"
                offset="2"
                class="x-small-font"
              >{{$t('trade.amount')}}({{tradepair.baseCode}})</van-col>
            </van-row>
            <van-row>
              <van-col span="11">
                <van-field
          ...
          <div class="item">
            ...
            <van-row>
              <van-col
                span="11"
                class="text-muted"
              >{{expectedTradePrice | cutTail}}&nbsp;{{tradepair.counterCode}}</van-col>
              <van-col span="11" offset="2">
                <vue-slider
        ...
        >
          <div
            class="single-btn margin-top margin-bottom"
          ...
          >
            <van-button
              ...
              class="buy-bg-image"
              style="border:none;"
              @click="toInputPassword"
              v-if="bsFlag === 'buy'"
            >{{$t('trade.buyingCode', {code: tradepair.baseCode})}}</van-button>
            <van-button
              size="large"
              round
              type="danger"
              ...
              style="border:none"
              @click="toInputPassword"
              v-else-if="bsFlag === 'sell'"
            >{{$t('trade.selling')}}{{tradepair.baseCode}}</van-button>
          </div>
...
export default {
  ...
  watch: {
    ...
    "form.price"() {
      if (this.form.price) {
        this.form.price =
          this.form.price.toString().match(/^\d*(\.?\d{0,7})/g)[0] || null;
      }
    },
    "form.amount"() {
  ...
  methods: {
    ...
    init() {
      ...
      let account = this.$store.state.account;
      if (account) {
        let tradepair = this.$collecitons.tradepair.findByAcctTypeAndAddress(
          account.type,
          account.address
        );
        ...
        this.bsIssuer = this.tradepair.counterIssuer
          ? this.tradepair.counterIssuer
          : "";
        this.$nextTick(() => {
    ...
    toBs(flag) {
      ...
      if (flag === "buy") {
        ...
        this.bsIssuer = this.tradepair.baseIssuer
          ? this.tradepair.baseIssuer
          : "";
      }
    },
    ...
    checkForm() {
      ...
      if (this.bsFlag === "buy") {
        if (
          Number(this.balances[this.bsCode + this.bsIssuer]) <
          Number(this.expectedTradePrice)
        ) {
    ...
    tradeTx(password) {
      ...
      this.$wallet
        .sendOffer(
          ...
          cryptor.decryptAES(this.currentAccount.secret, password),
          this.bsFlag
        )
        .then(ret => {
          if (ret) {
    ...
    viewMyBooks() {
      this.$refs.tradeLastBookPop.show(
        this.tradepair,
