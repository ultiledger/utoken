<template>
  ...
  <van-cell :center="true" v-if="item.txType !== '3' && item.txType !== '4'">
    <div class="item">
      ...
      <div class="content">
        <div
          class="detail-card-line small-font"
          v-if="item.txType === '1'"
        >{{item.from | longStrAbbr(8)}}</div>
        <div class="detail-card-line small-font" v-else>{{item.to | longStrAbbr(9)}}</div>
        <div
          class="x-small-font text-muted detail-card-line"
    ...
    <div v-if="showPercentage">
      <div class="text-right x-small-font" style="padding-right: 5px;">{{tip}}</div>
...
export default {
  ...
  computed: {
    ...
    showPercentage() {
      return (
        this.item.confirmations !== undefined &&
        this.item.confirmations <= this.maxConfirmations
      );
    },
    percentage() {
